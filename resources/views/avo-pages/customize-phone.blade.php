                ...
                <div class="StepsContentLeft">
                    <div class="wrapDescrip">
                        <form action="/save-phone-settings" method="post" name="form1" id="form1" >
                            ...
                            <br>
                            <div id="Prefix" style="margin:bottom:15px;display:none;">
                            </div>
...
@section('scripts')
    ...
    <script>
        ...
        $("select[name='country_code']").on('change', function()
        {
            ...
            $.ajax({
                ...
                success : function(data)
                {
                    ...
                    var html = '<div style="margin: 10px 0 5px 0;">'+
                    ...
                    {
                        if(data != '') {
                            html = '<div class="areaCode">'+
                                        '<p><span class="mediumBold">Please select the area code you\'d like:</span></p>'+
                                        '<select id="prefix_select" class="TollFPhone changeMtop"></select>'+
                                    '</div><br>';
                        }
                        $("#Prefix").html(html);
                        $("#Prefix select").html("<option value=''>Please Select</option>" + data);
                        $("#Prefix").show();
                        $("#numbers").find('select').html("<option value=''>Please Select</option>");
                        $("#numbers").hide();
                    } else {
                        if(data != '') {
        ...
        $("#Prefix").on('change', 'select', function()
        {
            $("input[type='submit']").hide();
            var prefix = $(this).val();
        ...
        $("#numbers").on('change', 'select', function()
        {
            if($(this).val() == '') {
                $("input[type='submit']").hide();
            } else {
