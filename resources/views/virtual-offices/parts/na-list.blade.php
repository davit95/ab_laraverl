	...
	<tr>	
		...
		@foreach($states as $counter => $state)
				...
				@foreach($state->active_cities as $city)
					<div class="city-header"><a href="{!! URL::action('VirtualOfficesController@getCityVirtualOffices', ['country_code' => $city->country_code, 'city-slug' => $city->us_state, 'city_id' => $city->name])!!}" class="style5" title="{{ $city->name }} virtual offices">{!! $city->name !!}</a></div>
				@endforeach
				@if($counter === 10)
