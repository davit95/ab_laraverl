	...
	<tr>
		...
		@foreach($states as $counter => $state)
				...
				@foreach($state->active_cities as $city)
					<div class="city-header"><a href="{!! URL::action('VirtualOfficesController@getCityVirtualOffices', ['country_code' => $city->country_code, 'city-slug' => $city->slug, 'city_id' => $city->id, 'us_state_id' => $city->us_state_id])!!}" class="style5" title="$CityData[0] virtual offices">{!! $city->name !!}</a></div>
				@endforeach
				@if($counter === 10)
