	...
	<tr>
		...
		@foreach($states as $key => $state)
				...
				@foreach($state->active_cities as $city)
					<div class="city-header"><a href="{!! URL::action('VirtualOfficesController@getCityVirtualOffices', ['country_code' => $city->country_code, 'city-slug' => $city->slug])!!}" class="style5" title="$CityData[0] virtual offices">{!! $city->name !!}</a></div>
				@endforeach
				@if($counter === 10)
